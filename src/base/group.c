...
static void      split_static_array(tag_elem_t *tags, UT_array **other_tags);
...
void neu_group_split_static_tags(neu_group_t *group, UT_array **other_tags)
{
    return split_static_array(group->tags, other_tags);
}
                           ...
                           neu_group_change_fn fn)
{
    if (group->timestamp != timestamp) {
        UT_array *other_tags = NULL;
        split_static_array(group->tags, &other_tags);
        fn(arg, group->timestamp, other_tags, group->interval);
    }
}
...
static void split_static_array(tag_elem_t *tags, UT_array **other_tags)
{
    ...
    HASH_ITER(hh, tags, el, tmp)
    {
        if (neu_tag_attribute_test(el->tag, NEU_ATTRIBUTE_SUBSCRIBE) ||
