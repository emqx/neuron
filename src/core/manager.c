...
static int manager_loop(enum neu_event_io_type type, int fd, void *usr_data)
{
    ...
    case NEU_REQ_GET_NODES_STATE: {
        ...
        neu_adapter_t *adapter_core = neu_node_manager_find(manager->node_manager, "default-dashboard");
    ...
    case NEU_REQ_GET_NODE_STATE: {
        if (neu_node_manager_find(manager->node_manager, header->receiver) ==
            ...
            neu_resp_get_node_state_t *resp = (neu_resp_get_node_state_t *) &header[1];
            neu_adapter_t *adapter_core = neu_node_manager_find(manager->node_manager, "default-dashboard");
            strcpy(resp->core_level,
                    neu_plugin_to_plugin_common(adapter_core->plugin)->log_level);
            forward_msg(manager, header, header->receiver);
    ...
    case NEU_REQ_UPDATE_LOG_LEVEL:
        ...
        if (cmd->core) {
            if (strcmp(header->receiver, "") == 0) {
                neu_adapter_t *adapter_core = neu_node_manager_find(manager->node_manager, "default-dashboard");
                strcpy(neu_plugin_to_plugin_common(adapter_core->plugin)->log_level,
                        cmd->log_level);
                neu_resp_error_t error = { 0 };
                neu_msg_exchange(header);
                header->type = NEU_RESP_ERROR;
