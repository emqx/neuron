...
static void manager_bind_adapter(nng_pipe p,
								 nng_pipe_ev ev,
								 void* arg)
{
	int rv = 0;
	...
	log_info("The manager will bind adapter(%s)",
			 neu_adapter_get_name(adapter));

	return;
}

static void manager_unbind_adapter(nng_pipe p,
								   nng_pipe_ev ev,
								   void* arg)
{
	int rv = 0;
	...
	adapter = (neu_adapter_t*)arg;
	log_info("The manager will unbind adapter(%s)",
			 neu_adapter_get_name(adapter));
...
static int manager_remove_adapter(neu_manager_t* manager,
	   							  neu_adapter_t* adapter)
{
	int rv = 0;
...
static int manager_stop_adapter(neu_manager_t* manager,
								neu_adapter_t* adapter)
{
	int rv = 0;
	size_t   msg_size;
