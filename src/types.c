...
neu_double_t 
neu_bytedec_toDouble(neu_bytedec_t b) {
    neu_word_t dec = 1;
...
neu_double_t 
neu_ubytedec_toDouble(neu_ubytedec_t ub) {
    neu_word_t dec = 1;
...
neu_double_t 
neu_worddec_toDouble(neu_worddec_t w) {
    neu_word_t dec = 1;
...
neu_double_t 
neu_uworddec_toDouble(neu_uworddec_t uw) {
    neu_word_t dec = 1;
...
neu_double_t 
neu_dworddec_toDouble(neu_dworddec_t dw) {
    neu_word_t dec = 1;
...
neu_double_t 
neu_udworddec_toDouble(neu_udworddec_t udw) {
    neu_word_t dec = 1;
...
neu_double_t 
neu_qworddec_toDouble(neu_qworddec_t qw) {
    neu_word_t dec = 1;
...
neu_double_t 
neu_uqworddec_toDouble(neu_uqworddec_t uqw) {
    neu_word_t dec = 1;
...
neu_string_t
neu_string_fromArray(char *array) {
    neu_string_t s; s.length = 0; s.charstr = NULL;
    if(!array)
        return s;
...
neu_string_t
neu_string_fromChars(const char *src) {
    neu_string_t s; s.length = 0; s.charstr = NULL;
    if(!src)
        return s;
    s.length = strlen(src);
...
neu_order_t
neu_string_isEqual(const neu_string_t *s1, const neu_string_t *s2) {
    ...
    return (is == 0) ? NEU_ORDER_EQ : ((is > 0) ? NEU_ORDER_MORE : NEU_ORDER_LESS);
}
...
neu_time_t 
neu_time_fromDateTime(neu_datetime_t dt) {
    const neu_udword_t mon_yday[2][12] = {
        {0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334},
        {0, 31, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335},
        };
    neu_word_t leap = ((dt.tm_year) % 4 == 0 && ((dt.tm_year) % 100 != 0 || (dt.tm_year) % 400 == 0));
...
neu_boolean_t
neu_uuid_isEqual(const neu_uuid_t *u1, const neu_uuid_t *u2) {
...
neu_boolean_t
neu_nodeid_isNull(const neu_nodeid_t *n) {
...
neu_order_t
neu_nodeid_order(const neu_nodeid_t *n1, const neu_nodeid_t *n2) {
    return (neu_string_isEqual(&n1->identifier, &n2->identifier));
}

const neu_nodeid_t NEU_NODEID_NULL = {{{0, NULL}, {0, NULL}}, {0, NULL}};
...
void
neu_variable_setScalar(neu_variable_t *v, void *p,
...
void 
neu_variable_setArray(neu_variable_t *v, void *array,
