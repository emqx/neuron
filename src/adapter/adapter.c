...
static int persister_singleton_load_plugins(neu_adapter_t *adapter)
{
    ...
    if (0 != rv) {
        nlog_error("%s failed to load plugin infos", adapter->name);
        return -1;
    }
    VECTOR_FOR_EACH(plugin_infos, iter)
    {
        ...
        nlog_info("%s load plugin %s , lib:%s", adapter->name, ok_or_err,
                  *plugin_info);
    }
                                                 ...
                                                 neu_node_id_t  node_id)
{
    ...
    if (NULL == tag_tbl) {
        nlog_error("%s fail get datatag table of %s", adapter->name,
                   adapter_name);
    ...
    VECTOR_FOR_EACH(group_config_infos, iter)
    {
        ...
        const char *ok_or_err = (0 == rv) ? "success" : "fail";
        nlog_info("%s %s load group config %s interval:%" PRId64, adapter->name,
                                              ...
                                              msg_type_e     event)
{
    ...
    if (0 != rv) {
        nlog_error("%s unable to get plugin infos", adapter->name);
        return rv;
    }
                                                 ...
                                                 void *         arg)
{
    ...
    if (MSG_EVENT_DEL_GRP_CONFIG != event) {
        ...
        if (0 != rv) {
            nlog_error("%s failed to store group config infos", adapter->name);
        }
    } else {
        ...
        rv = neu_persister_delete_group_config(persister, adapter_name,
                                               group_config_name);
        if (0 != rv) {
            nlog_error("%s failed to del group config infos", adapter->name);
        }
    }
                                                    ...
                                                    neu_node_id_t  node_id)
{
    ...
    if (0 != rv) {
        nlog_error("%s fail store adapter:%s subscription infos", adapter_name,
                   adapter->name);
    }
                           ...
                           neu_response_t **p_result)
{
    ...
    case NEU_REQRESP_GET_NODE_BY_ID: {
        ADAPTER_RESP_CMD(
            adapter, cmd, neu_reqresp_node_info_t, neu_cmd_get_node_by_id_t, rv,
            NEU_REQRESP_NODE_INFO, p_result, {
                ret = calloc(1, sizeof(neu_reqresp_node_info_t));

                ret->result = neu_manager_get_node_by_id(
                    adapter->manager, req_cmd->node_id, &ret->node_info);
            });
        break;
    }
    ...
    case NEU_REQRESP_GET_NODES: {
        ADAPTER_RESP_CMD(
            ...
            NEU_REQRESP_NODES, p_result, {
                ...
                if (ret == NULL) {
                    nlog_error("Failed to allocate result of get nodes");
    ...
    case NEU_REQRESP_GET_GRP_CONFIGS: {
        ADAPTER_RESP_CMD(
            ...
            rv, NEU_REQRESP_GRP_CONFIGS, p_result, {
                ...
                if (ret == NULL) {
                    nlog_error("Failed to allocate result of get grp configs");
                    rv = -1;
                    free(result);
                    break;
                }
    ...
    case NEU_REQRESP_GET_DATATAGS: {
        ADAPTER_RESP_CMD(
            ...
            NEU_REQRESP_DATATAGS, p_result, {
                ...
                if (ret == NULL) {
                    nlog_error("Failed to allocate result of get datatags");
                    rv = -1;
                    free(result);
                    break;
                }
    ...
    case NEU_REQRESP_SELF_NODE_ID: {
        ADAPTER_RESP_CMD(
            ...
            NEU_REQRESP_NODE_ID, p_result, {
                ...
                if (ret == NULL) {
                    nlog_error("Failed to allocate result of get node id");
                    rv = -1;
                    free(result);
                    break;
                }
    ...
    case NEU_REQRESP_SELF_NODE_NAME: {
        ADAPTER_RESP_CMD(
            ...
            NEU_REQRESP_NODE_NAME, p_result, {
                ...
                if (ret == NULL) {
                    nlog_error("Failed to allocate result of get node name");
                    rv = -1;
                    free(result);
                    break;
                }
