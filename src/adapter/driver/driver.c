                   ...
                   neu_dvalue_t value)
{
    ...
    if (value.type == NEU_TYPE_ERROR && tag == NULL) {
        ...
        if (g != NULL) {
            ...
            driver->adapter.metrics.driver.tag_reads_total += utarray_len(tags);
            driver->adapter.metrics.driver.tag_errors_total +=
                utarray_len(tags);
            utarray_free(tags);
        }
    } else {
        neu_driver_cache_update(driver->cache, group, tag,
                                driver->adapter.timestamp, value);
        driver->adapter.metrics.driver.tag_reads_total++;
        driver->adapter.metrics.driver.tag_errors_total +=
