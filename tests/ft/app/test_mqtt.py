...
class TestMQTT:
    ...
    @description(
        given="MQTT node",
        when="read an error tag and configurate mqtt node with skip_err",
        then="broker should not receive error data on upload topic",
    )
    def test_mqtt_upload_skip_err(self, mocker, conf_skip_err):
        api.node_setting_check(NODE, conf_skip_err)
        api.node_ctl(NODE, config.NEU_CTL_START)
