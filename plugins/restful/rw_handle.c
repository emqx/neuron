...
void handle_read_paginate(nng_aio *aio)
{
    ...
    NEU_PROCESS_HTTP_REQUEST_VALIDATE_JWT(
        aio, neu_json_read_req_t, neu_json_decode_read_paginate_req, {
            int                  ret    = 0;
            neu_reqresp_head_t   header = { 0 };
            neu_req_read_group_t cmd    = { 0 };
            int                  err_type;
...
void handle_read_paginate_resp(nng_aio *aio, neu_resp_read_group_t *resp)
{
    ...
    if (resp->is_error) {
        ...
        utarray_foreach(resp->tags, neu_resp_tag_value_meta_t *, tag_value)
        {
            if (tag_value->value.type == NEU_TYPE_ERROR) {
                utarray_push_back(filtered_tags, tag_value);
            }
        }
    ...
    if (resp->current_page > 0 && resp->page_size > 0) {
        ...
        if (start_index >= total_tags) {
            api_res.n_tag = 0;
            api_res.tags  = calloc(1, sizeof(neu_json_read_resp_tag_t));
        } else {
            ...
            api_res.tags =
                calloc(api_res.n_tag, sizeof(neu_json_read_resp_tag_t));

            for (unsigned int i = start_index; i < end_index; i++) {
                neu_resp_tag_value_meta_t *tag_value =
                    (neu_resp_tag_value_meta_t *) utarray_eltptr(resp->tags, i);
                neu_tag_value_to_json(tag_value,
