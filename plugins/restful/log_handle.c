...
// static int get_log(const char *fname, uint64_t since, uint64_t until, int
// level, uint32_t page, uint32_t page_size, vector_t *lines, size_t *line_tot);
...
void handle_get_log(nng_aio *aio)
{
    ...
    // if (NULL != level_str && 0 != string_to_log_level(level_str, len,
    // &level)) { NEU_JSON_RESPONSE_ERROR(NEU_ERR_PARAM_IS_WRONG, {
    // http_response(aio, error_code.error, result_error);
    //});
    // return;
    //}
