                      ...
                      mqtt_config_t *config)
{
    ...
    ret = neu_parse_param(setting, NULL, 1, &skip_err);
    if (0 != ret) {
        plog_notice(plugin, "setting skip_err failed");
    }
    ...
    config->skip_err                 = skip_err.v.val_bool;
    ...
    plog_notice(plugin, "config skip-err: %d", config->skip_err);
