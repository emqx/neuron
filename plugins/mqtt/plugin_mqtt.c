                                   ...
                                   void *context)
{
    ...
    if (NEU_PARSE_OP_READ_TAG_GROUP_LIST == req->function) {
        message_handle_read_tag_group_list_handle(
            (struct neu_paser_read_tag_group_list_req *) result);
    }

    if (NEU_PARSE_OP_READ == req->function) {
        message_handle_read(req);
    }

    if (NEU_PARSE_OP_WRITE == req->function) {
        message_handle_write_handle((struct neu_parse_write_req *) result);
    }
    ...
    if (NEU_PARSE_OP_ADD_TAGS == req->function) {
        message_handle_add_tag_handle((struct neu_parse_add_tags_req *) req);
    }
...
static int mqtt_plugin_init(neu_plugin_t *plugin)
{
    ...
    message_handle_set_plugin_common(&plugin->common);
    message_handle_set_paho_client(plugin->paho);
    message_handle_set_event_id(&plugin->new_event_id);
    return 0;
}
