...
#include "modbus_point.h"
                ...
                /**
                 * modbus_point.c
                 * 本文件实现了 Modbus 点（数据点）相关的功能，包括：
                 * - 将通用数据标签转换为 Modbus 协议点
                 * - 处理点的读写操作
                 * - 对点进行排序与分组
                 * - 处理不同数据类型的字节序转换
                 */
        ...
        // 用于排序的比较函数（读操作）
        static int tag_cmp(neu_tag_sort_elem_t * tag1,
                           neu_tag_sort_elem_t * tag2);
        // 判断标签是否可以放入同一组（读操作）
        static bool tag_sort(neu_tag_sort_t * sort, void *tag,
                             void *tag_to_be_sorted);
        // 用于排序的比较函数（写操作）
        static int tag_cmp_write(neu_tag_sort_elem_t * tag1,
                                 neu_tag_sort_elem_t * tag2);
        // 判断标签是否可以放入同一组（写操作）
        ...
        int modbus_tag_to_point(const neu_datatag_t *tag, modbus_point_t *point)
        {
            ...
            case MODBUS_AREA_HOLD_REGISTER:
                ...
                if (point->type == NEU_TYPE_BYTES &&
                    point->option.bytes.length <= 0) {
                    return NEU_ERR_TAG_ADDRESS_FORMAT_INVALID;
                }
            ...
            case NEU_TYPE_BYTES:
                if (point->area == MODBUS_AREA_COIL ||
                    ...
                    point->n_register = point->option.bytes.length / 2 +
                        point->option.bytes.length % 2;
                }
                break;
            default:
                return NEU_ERR_TAG_TYPE_NOT_SUPPORT;
        ...
        /**
         * 计算给定类型数据所需的字节数并进行字节序转换
         *
         * @param type 数据类型
         * @param value 数据值，会被修改为网络字节序
         * @param option 地址选项
         * @param endianess_64 64位数据字节序
         * @param default_tag_endian_64 是否使用默认64位字节序
         * @return 字节数
         */
        int cal_n_byte(
            int type, neu_value_u *value, neu_datatag_addr_option_u option,
         ...
         * @return 排序后的写命令列表
         */
        modbus_write_cmd_sort_t *modbus_write_tags_sort(
            UT_array * tags, modbus_endianess_64 endianess_64)
        {
            neu_tag_sort_result_t *result =
            ...
            sort_result->n_cmd = result->n_sort;
            sort_result->cmd =
                calloc(result->n_sort, sizeof(modbus_write_cmd_t));
            for (uint16_t i = 0; i < result->n_sort; i++) {
                modbus_point_write_t *tag =
                    *(modbus_point_write_t **) utarray_front(
                        result->sorts[i].tags);
                struct modbus_sort_ctx *ctx = result->sorts[i].info.context;

                int num_tags = utarray_len(result->sorts[i].tags);
                sort_result->cmd[i].bytes =
                    calloc(num_tags, sizeof(neu_value_u));
                int      n_byte = 0, n_byte_tag = 0;
                uint8_t *data_bit = calloc((num_tags + 7) / 8, sizeof(uint8_t));
                int      k        = 0;
                utarray_foreach(result->sorts[i].tags, modbus_point_write_t **,
                                tag_s)
                {
                ...
                if ((*(modbus_point_write_t **) utarray_front(
                         result->sorts[i].tags))
                        ->point.area == MODBUS_AREA_COIL) {
                                   ...
                                   void *tag_to_be_sorted)
        {
            ...
            modbus_point_write_t *t2 =
                (modbus_point_write_t *) tag_to_be_sorted;
            struct modbus_sort_ctx *ctx = NULL;
